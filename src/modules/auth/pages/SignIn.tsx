...
import { loginFormDataInterface } from '@/interface/FunctionParams.interface';
...
import { signInApiFunction, verifyUsersLoginStatus } from '@/utils/api/api';
import { MAX_SIGN_IN_ATTEMPT } from '@/utils/constants/global.constants';
...
import { ValidateSignInForm } from '@/utils/validation/auth.validation';
...
const INITIAL_FORM_DATA: loginFormDataInterface = {
   email: '',
   password: '',
   rememberMe: false,
};
...
function SignIn() {
   ...
   const [formData, setFormData] =
      useState<loginFormDataInterface>(INITIAL_FORM_DATA);
   ...
   const signInApiHandlerFunction = useDebounce(async () => {
      ...
      if (res?.success) {
         ...
         if (!res?.data?.organization_created) {
            navigate(
               `/onboarding?organization_id=${res?.data?.organization_id}`
            );
         } else {
            navigate(
   ...
   const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const { isValid, errors } = ValidateSignInForm({ value: formData });
                  ...
                  <div className='login-form w-full h-full relative z-20 flex items-center justify-center'>
                     <div className='flex flex-col gap-8 sm:gap-10 items-start justify-start w-full max-w-[400px] p-4 md:p-0'>
                        ...
                        >
                              ...
                              <div className='w-full flex items-center justify-between'>
                                 <div className='flex items-center justify-start gap-1.5'>
                                    <Input
                                       ...
                                       disabled={loading}
                                       checkboxLabel={
                                          <span className='ml-2 text-black font-light text-sm font-inter'>
                                             Remember Me
                                          </span>
                                       }
                                    />
                           ...
                           <div className='w-full grid grid-cols-1 pt-5'>
                              <Button
                                 type='submit'
